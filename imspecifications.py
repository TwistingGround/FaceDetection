...
def specifications(img_path, img, x, y, h):   #puts text on image
    attributes = DeepFace.analyze(img_path, actions = ['gender', 'age', 'race', 'emotion'])
    ...
    if attributes:
                for specs in attributes:
                    ...
                    c = 60
                    while i<len(text):
                            cv2.putText(img, text[i], (x,y+h+c), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 1) #puts each attribute in its own line
                            i+=1
                            c+=30
    else:
